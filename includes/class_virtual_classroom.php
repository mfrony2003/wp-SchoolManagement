...
class mj_smgt_virtual_classroom
{
	...
	public function mj_smgt_create_meeting_in_zoom($data)
	{
	    ...
	    try 
	    {
        	...
        	if($data['action']=='edit')
			{
				school_append_audit_log(''.esc_html__('Update Virtual Classroom Detail','hospital_mgt').'',null,get_current_user_id(),'edit');
			...
			else
			{
				if($meeting_response)
				{
					school_append_audit_log(''.esc_html__('Add New Virtual Classroom','hospital_mgt').'',null,get_current_user_id(),'insert');
	...
	{
		school_append_audit_log(''.esc_html__('Delete Virtual Classroom Meeting','hospital_mgt').'',null,get_current_user_id(),'delete');
		global $wpdb;
