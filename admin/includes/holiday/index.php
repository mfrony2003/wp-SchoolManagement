	...
	$('#holiday_form').validationEngine({promptPosition : "bottomLeft",maxErrorsPerField: 1});
	  ...
	  $('#date').datepicker({
        ...
        onSelect: function (selected) {
            ...
            $("#end_date_new").datepicker("option", "minDate", dt);
        }
	});
			...
			else
			{
				...
				if(!empty($results))
				{
					foreach($results as $retrive_data)
					{
						...
						$title = mj_smgt_popup_category_validation(stripslashes($_POST['holiday_title']));
						$text = mj_smgt_address_description_validation(stripslashes($_POST['description']));
	...
	<div class="main_list_margin_15px"><!-- main_list_margin_15px -->
		...
		switch($message)
		{
			...
			case '3':
				...
				break;
			case '4':
				$message_string = esc_attr__('Holiday Approve Successfully.','school-mgt');
				break;
		}
